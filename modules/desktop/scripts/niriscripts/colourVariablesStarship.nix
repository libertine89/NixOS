...
pkgs.writeShellScriptBin "generate-color-variables-starship" ''
  ...
  OUT_STARSHIP="$HOME/NixOS/modules/core/starship/starship.toml"
  ...
  generate_STARSHIP() {
      local FILE="$OUT_STARSHIP"

      local palette_block
      palette_block=$(cat <<EOF
[palettes.dms]
color1 = "$(extract_color '.colors.dark.on_primary')" # Darkest Bar Color
color2 = "$(extract_color '.colors.dark.on_primary_fixed_variant')" # Middle Bar Color
color3 = "$(extract_color '.colors.dark.inverse_primary')"  # Lightest Bar Color
color4 = "#000000" # Spare
color5 = "$(extract_color '.colors.dark.on_secondary_container')"  # Static Seperator Color
color6 = "$(extract_color '.dank16.color2.dark')" # Git Lines Added Color
color7 = "$(extract_color '.dank16.color9.dark')" # Git Lines Removed Color
color8 =  "$(extract_color '.colors.dark.on_surface')" # Text Color
cursor1 = "#FF5C57" # Error Symbol
cursor2 = "#FFFFFF" # Sucess Symbol
cursor3 = "#6905F7" # Vim CMD Symbol
os = "#FFFFFF"
...
EOF
      ...
      if [[ ! -f "$FILE" ]]; then
          ...
          echo "$palette_block" > "$FILE"
          return
      fi

      # Replace existing [palettes.dms] block
      if grep -q '^\[palettes\.dms\]' "$FILE"; then
